...
export function emptyState() {
  return {schemaVersion:2,patient:{preferredName:'',fullName:''},medications:[],medicationPending:null,memoryPending:null,
    memories:[],events:[],openLoops:[],appointments:[],debug:[],lastUpdated:null};
}
export function validateState(input) {
  ...
  if(input.appointments!==undefined&&(!Array.isArray(input.appointments)||input.appointments.some(a=>!a||typeof a.id!=='string'||typeof a.with!=='string'||!Number.isFinite(Date.parse(a.startsAt)))))throw Error('Invalid appointments');
